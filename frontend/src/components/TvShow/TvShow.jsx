...
function TvShow(props) {
  ...
  const getGenreNames = (genre_ids, genres) => {
    if (!genre_ids || genre_ids.length == 0) {
      ...
      return props.movie.genre_ids.map(
        (id) => props.genres.find((genre) => genre.id === id)?.name,
      );
  };
  ...
  return (
    ...
    <div className="movie-container" onClick={handleMovieClick}>
      {/* affiche l'image*/}
      <img src={posterSrc} alt={'Affiche'} className="movie-poster" />
      ...
      <div className="movie-details-wrapper">
        <div className="movie-details-box">
          <div className="movie-genres">{genreNames.join(', ')}</div>
          <p className="movie-synopsis">
            <strong>Overview</strong> : {cutOverview(props.movie.overview)}
          </p>
